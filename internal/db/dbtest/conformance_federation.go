...
func checkFederationTeammateMaterializationValidation(t *testing.T, store db.Storage) error {
	t.Helper()
	ctx := context.Background()
	...
	for index, fixture := range []struct {
		...
		for _, event := range fixture.events(t, project, issueUID) {
		...
		assert.ErrorContains(t, err, "teammate", fixture.name)
		_, err = store.IssueByUID(ctx, issueUID, db.IncludeDeletedYes)
		assert.ErrorIs(t, err, db.ErrNotFound, "%s must roll back every projection write", fixture.name)
	}
	return nil
}
